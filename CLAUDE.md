...
Hierarchical resolver (`packages/secrets/resolver`) walks `user -> team -> tenant` under `/ark/<tid>/...` at dispatch time. See `docs/secrets-usage.md` for the path convention, CLI scope flags, and the precedence rules.
