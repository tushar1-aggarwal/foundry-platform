...
run_pr_stage() {
  ...
  # Set authed origin so push works
  git -C "$WORKDIR" remote set-url origin "$AUTHED_URL"

  local push_out push_exit
  push_out=$(git -C "$WORKDIR" push -u origin "$ISO_BRANCH" 2>&1)
  push_exit=$?
  log "push exit=$push_exit"
  echo "$push_out" > "$stage_dir/push.log"

  local pr_url="" pr_body pr_resp pr_http
  if [ "$push_exit" -eq 0 ]; then
    # Bitbucket REST API: workspace=paytmteam, repo=foundry-test-repo
    pr_body=$(jq -n \
      --arg title "Isolation test: $ISO_SUMMARY" \
      --arg src "$ISO_BRANCH" \
      --arg desc "Generated by pod-isolation driver (session=$ISO_SESSION_ID)" \
      '{title:$title, description:$desc, source:{branch:{name:$src}}, destination:{branch:{name:"master"}}, close_source_branch:true}')
    pr_resp=$(curl -sS -u "$BITBUCKET_USERNAME:$BITBUCKET_TOKEN" \
      -H "Content-Type: application/json" \
      -X POST "https://api.bitbucket.org/2.0/repositories/paytmteam/foundry-test-repo/pullrequests" \
      -d "$pr_body" -w "\nHTTP %{http_code}")
    pr_http=$(echo "$pr_resp" | tail -1 | awk '{print $2}')
    pr_url=$(echo "$pr_resp" | head -n -1 | jq -r '.links.html.href // ""' 2>/dev/null || echo "")
    echo "$pr_resp" > "$stage_dir/bb_response.log"
    log "PR API http=$pr_http url=$pr_url"
  fi

  end_ts=$(date +%s%3N)
  local duration_ms=$((end_ts - start_ts))
  local exit_code=0
  if [ "$push_exit" -ne 0 ] || [ -z "$pr_url" ]; then
    exit_code=1
  fi
  ...
  jq -n \
    ...
    '{stage:"pr", exit_code:$exit_code, duration_ms:$duration_ms, push_exit:$push_exit, branch:$branch, pr_url:$pr_url}' \
    > "$stage_dir/outputs.json"
