...
export const scopingOverrides = sqliteTable(
  "scoping_overrides",
  {
    id: text("id").primaryKey(),
...
// ── skills ────────────────────────────────────────────────────────────────
//
// Skill Hub central registry. See docs/skillhub-rfc.md §3.
//
// tenant_id is NULLABLE: user-scope rows carry NULL so a user in multiple
// tenants sees their personal skills across all of them (consultant
// pattern).
//
// **IMPORTANT — drizzle-kit will erase the CHECK constraint.** This table
// has a `ck_skills_visibility_scope` CHECK constraint enforcing the
// visibility/scoping-column invariant. The constraint is added in
// migration 022_skills_sqlite.ts and lives at the DB level only —
// drizzle's table builder doesn't model CHECK constraints, so this
// declaration doesn't reflect it. Do NOT regenerate migrations for this
// table with `drizzle-kit generate` (or any drizzle migrate command); the
// autogenerated SQL will silently drop the CHECK constraint. Modify this
// table via a hand-written migration that preserves the constraint.
...
export const skills = sqliteTable(
  ...
  (t) => ({
    ...
    idxUniqTenant: uniqueIndex("idx_skills_tenant_name_live")
      ...
      .where(
        sql`${t.deletedAt} IS NULL AND ${t.visibility} = 'tenant' AND ${t.teamId} IS NULL AND ${t.ownerUserId} IS NULL`,
      ),
...
// ── skill_versions ────────────────────────────────────────────────────────
//
// Append-only body history. Every `skill/put` that changes the body writes
// a row here BEFORE updating the live `skills` row, in the same transaction.
// Source of truth for the 3-way merge ancestor body (RFC §7).
