...
describe("garbageCollectComputeIfTemplate", () => {
  it("returns false when computeName is null/undefined/missing", async () => {
    expect(await garbageCollectComputeIfTemplate(depsFromApp(app), null)).toBe(false);
    expect(await garbageCollectComputeIfTemplate(depsFromApp(app), undefined)).toBe(false);
    expect(await garbageCollectComputeIfTemplate(depsFromApp(app), "no-such")).toBe(false);
  });

  it("never gc's persistent-lifecycle (local) compute", async () => {
    expect(await garbageCollectComputeIfTemplate(depsFromApp(app), "local")).toBe(false);
  ...
  it("gc's template-lifecycle (k8s) compute when no sessions reference it", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "test-k8s");
    expect(gc).toBe(true);
  ...
  it("skips gc when a live session still references the compute", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "busy-k8s");
  ...
  it("gc's after the referencing session reaches a terminal state", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "ephemeral-k8s");
    expect(gc).toBe(true);
  ...
  it("local + docker (template isolation over persistent kind) gets gc'd", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "local-docker");
    expect(gc).toBe(true);
  ...
  it("cloned_from rows are GC'd even when the kind is persistent", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "ec2-clone");
    expect(gc).toBe(true);
  ...
  it("persistent row WITHOUT cloned_from is NOT GC'd", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "ec2-persistent");
    expect(gc).toBe(false);
  ...
  it("DOES gc a clone row (force-delete bypass for clones is the path)", async () => {
    ...
    const gc = await garbageCollectComputeIfTemplate(depsFromApp(app), "stub-clone");
    expect(gc).toBe(true);
