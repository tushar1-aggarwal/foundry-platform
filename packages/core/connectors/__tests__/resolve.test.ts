...
beforeAll(async () => {
  ...
  await app.boot();
  await app.flows.save("flow-with-connectors", {
...
describe("flowConnectorsFor", () => {
  test("returns the connectors array from the flow YAML", async () => {
    expect(await flowConnectorsFor(app, "flow-with-connectors")).toEqual(["github", "jira"]);
  });

  test("returns [] for unknown flow", async () => {
    expect(await flowConnectorsFor(app, "does-not-exist")).toEqual([]);
  });

  test("returns [] for undefined flow name", async () => {
    expect(await flowConnectorsFor(app, undefined)).toEqual([]);
  });
});
...
describe("collectMcpEntries", async () => {
  test("returns empty array when no runtime or flow connectors declared", async () => {
    ...
    const entries = await collectMcpEntries(app, session, {});
  ...
  test("applied connectors are persisted to session.config.applied_connectors", async () => {
    const session = await app.sessions.create({ summary: "x", flow: "flow-with-connectors" });
    await collectMcpEntries(app, session, { flowConnectors: ["github"], sessionConnectors: ["jira"] });
