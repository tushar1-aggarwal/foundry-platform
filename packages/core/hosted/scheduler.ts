 ...
 *
 * Scheduling strategy:
 *   1. Check tenant policy (compute axes allowed, concurrency limit).
 *   2. Determine the (compute_kind, isolation_kind) pair from the session
 *      compute or the tenant default.
...
import type { Session, ComputeAxes } from "../../types/index.js";
...
export class SessionScheduler {
  ...
  async schedule(session: Session, tenantId?: string): Promise<WorkerNode> {
    ...
    // 2. Determine the (compute_kind, isolation_kind) pair
    let axes: ComputeAxes | null = null;
    if (session.compute_name) {
      axes = await this._resolveComputeAxes(session.compute_name, tid);
    }
    if (!axes && policy) {
      axes = policy.default_compute;
    }

    // 3. Validate the pair is allowed
    if (axes && this.policyManager && !(await this.policyManager.isComputeAllowed(tid, axes))) {
      const allowed =
        policy?.allowed_compute && policy.allowed_compute.length > 0
          ? policy.allowed_compute.map((a) => `${a.compute_kind}/${a.isolation_kind}`).join(", ")
          : "all";
      throw new Error(
        `Compute "${axes.compute_kind}/${axes.isolation_kind}" not allowed for tenant "${tid}". Allowed: ${allowed}`,
      );
    }

    // 4. Find or provision a worker
    return this._findOrProvisionWorker(session, tid, axes, policy);
  }
  ...
  private async _findOrProvisionWorker(
    session: Session,
    tenantId: string,
    axes: ComputeAxes | null,
    policy: TenantComputePolicy | null,
  ): Promise<WorkerNode> {
    const registry = this.app.workerRegistry;

    // Try existing idle worker for this tenant and compute kind
    if (axes) {
      const tenantWorkers = await registry.getAvailable({ tenantId, computeName: axes.compute_kind });
      if (tenantWorkers.length > 0) return this._pickBest(tenantWorkers);

      // Try any available worker with a matching compute kind
      const anyKind = await registry.getAvailable({ computeName: axes.compute_kind });
      if (anyKind.length > 0) return this._pickBest(anyKind);
    }
    ...
    if (policy && axes) {
      return this._provisionFromPool(session, tenantId, axes, policy);
    }
  ...
  private async _provisionFromPool(
    ...
    // Find the pool config for this (compute_kind, isolation_kind) pair
    const poolRef = policy.compute_pools.find(
      (p) => p.compute.compute_kind === axes.compute_kind && p.compute.isolation_kind === axes.isolation_kind,
    );
    if (!poolRef) {
      throw new Error("No workers available for scheduling");
    }
    ...
    // Check pool limits
    const registry = this.app.workerRegistry;
    const active = (await registry.list({ tenantId })).filter((w) => w.compute_name === axes.compute_kind).length;
    if (active >= poolRef.max) {
      throw new Error(`Pool "${poolRef.pool_name}" at max capacity (${poolRef.max})`);
    }

    // Look up the registered Compute impl for this kind.
    const computeImpl = this.app.getCompute(axes.compute_kind);
    if (!computeImpl) {
      throw new Error(`No compute impl registered for "${axes.compute_kind}/${axes.isolation_kind}"`);
    }

    // Create compute record
    const computeName = `${tenantId}-${axes.compute_kind}-${axes.isolation_kind}-${Date.now()}`;
  ...
  /**
   * Resolve the (compute_kind, isolation_kind) pair from a compute record.
   *
   ...
   * reading.
   */
  private async _resolveComputeAxes(computeName: string, tenantId: string): Promise<ComputeAxes | null> {
    ...
    return { compute_kind: compute.compute_kind, isolation_kind: compute.isolation_kind };
  }
