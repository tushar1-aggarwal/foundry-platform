...
// CLAUDE.md mandates 180s per E2E test + bail on first failure. The 50/100-
// request load tests below can exceed the playwright global 60s under cold CI.
...
test("SSE stream tolerates rapid connect/disconnect cycles", async () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ac.abort();
    // Give the cancel callback time to drain the client from the set. 50ms
    // is too short under CI CPU contention -- 250ms gives the server's
    // cancel handler room to finish before the next iteration.
